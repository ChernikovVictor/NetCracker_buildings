...
public class List implements Serializable
{
    ...
    public Space[] getSpaces() {
        ...
        for (Floor floor : floors) {
            Space[] spacesOnFloor = floor.getSpaceArray();
            for (Space space : spacesOnFloor) {
                spaces[index] = space;
                index++;
            }
        }
    ...
    public String toString() {
        if (head == null)
            return "";
        StringBuilder result = new StringBuilder(", " + head.floor.toString());
        int count = 1;
        Node currentNode = head.next;
        while (currentNode != head) {
            result.append(", ").append(currentNode.floor.toString());
            currentNode = currentNode.next;
            count++;
        }
